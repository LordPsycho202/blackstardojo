...
/***************************************************************************
 *                            bookie_allstats.php [English]
 *                              -------------------
 *     begin                : Sun Oct 17 2004
 *     copyright            : (C) 2004 Majorflam
 *     email                : majorflam@blueyonder.co.uk
 *     Website              : http://www.majormod.com
 *
 *     $Id: bookie_allstats.php,v 2.0.6 
 *
 ****************************************************************************/
...
// set the config
$new_bet_img = '<a href="' . append_sid("bookies.$phpEx?mode=placebet") . '"><img src="' . $images['icon_bookie_place_bet'] . '" title="' . $lang['icon_bookie_place_bet'] . '" alt="' . $lang['icon_bookie_place_bet'] . '" border="0"></a>';
$your_stats = '<a href="' . append_sid('bookie_yourstats.'.$phpEx) . '"><img src="' . $images['icon_bookie_yourstats'] . '" title="' . $lang['bookie_yourstats_title'] . '" alt="' . $lang['bookie_yourstats_title'] . '" border="0"></a>';
...
if (!$userdata['session_logged_in'] && $user_id == ANONYMOUS)
{
	...
	header($header_location . append_sid("login.$phpEx?redirect=bookie_allstats.$phpEx", true));
	exit;
}
...
{	case 'date':
	...
	$sql = "SELECT * FROM " . BOOKIE_BETS_TABLE . " ORDER BY $order_by";
...
{ 	
	...
	$sql_a = "SELECT username FROM " . USERS_TABLE . " WHERE user_id='$bet_userID'";
	$result_a = $db->sql_query($sql_a);
	$row_a = $db->sql_fetchrow($result_a);
	$bet_username = $row_a['username'];
		
	$username='<a href=' . append_sid("bookie_yourstats.$phpEx?viewmode=viewuser&u=$bet_userID") . ' class="nav">' . $bet_username . '</a>';
...
$total_won_stat=0;
$total_lost_stat=0;
$bookie_netpos_stat=0;
$stats_available=0;

$sql=" SELECT * FROM " . BOOKIE_STATS_TABLE;
if ( !($result = $db->sql_query($sql)) ) 
{ 
...
//
$sql=" SELECT count(*) AS total_user_wins FROM " . BOOKIE_BETS_TABLE . "
WHERE win_lose > 0
AND checked=1
";
if ( !($result = $db->sql_query($sql)) ) 
...
$sql=" SELECT count(*) AS total_user_lose FROM " . BOOKIE_BETS_TABLE . "
WHERE win_lose < 0
AND checked=1
";
if ( !($result = $db->sql_query($sql)) ) 
...
$sql=" SELECT count(*) AS user_refund FROM " . BOOKIE_BETS_TABLE . "
WHERE win_lose = 0
AND checked=1
";
if ( !($result = $db->sql_query($sql)) ) 
...
$sql=" SELECT count(*) AS user_pending FROM " . BOOKIE_BETS_TABLE . "
WHERE checked=0
";
if ( !($result = $db->sql_query($sql)) ) 
{ 
...
$sql=" SELECT user_id, bet FROM " . BOOKIE_BETS_TABLE . "
...
$sql=" SELECT user_id, win_lose FROM " . BOOKIE_BETS_TABLE . "
ORDER BY win_lose DESC LIMIT 1";
if ( !($result = $db->sql_query($sql)) ) 
{ 
	message_die(GENERAL_ERROR, 'Error in building statistics', '', __LINE__, __FILE__, $sql); 
}
...
$total_spend=0;
$sql="SELECT bet FROM " . BOOKIE_BETS_TABLE;
if ( !($result = $db->sql_query($sql)) ) 
{ 
	message_die(GENERAL_ERROR, 'Error in building statistics', '', __LINE__, __FILE__, $sql); 
}
...
$all_complete_stats=sprintf($lang['bookie_your_complete_allstats'], $total_user_bets, $user_wins, $user_lose, $stakes_refund, number_format($total_won_stat), $board_config['points_name'], number_format($total_lost_stat), $board_config['points_name'], number_format($bookie_netpos_stat), $board_config['points_name'], '<a href="' . append_sid("bookie_allstats.$phpEx?viewmode=viewuser&amp;u=$big_bet_id&amp;mode=stake&amp;order=DESC") . '" class="nav">' . number_format($big_bet) . '</a>',
 $board_config['points_name'], '<a href="' . append_sid("bookie_allstats.$phpEx?viewmode=viewuser&amp;u=$max_winnings_id&amp;mode=winnings&amp;order=DESC") . '" class="nav">' . number_format($max_winnings) . '</a>', $board_config['points_name'], ( $min_winnings < 0 ) ? '<a href="' . append_sid("bookie_allstats.$phpEx?viewmode=viewuser&amp;u=$min_winnings_id&amp;mode=winnings&amp;order=ASC") . '" class="nav">' . number_format( ($min_winnings*-1) ) . '</a>' : 0, $board_config['points_name'], number_format($total_spend), $board_config['points_name'], $user_pending);

	$sql = 'SELECT count(*) AS total 
	FROM ' . BOOKIE_BETS_TABLE . " ORDER BY time DESC
	"; 
	if ( !($result = $db->sql_query($sql)) ) 
	{ 
