...
if ( empty($action) )
{
	...
	if ( $sql_count )
	{
		for ($i = 0; $i < $sql_count; $i++)
		{
			...
			$name = ( $synth && $money ) ? '<a href="' . append_sid('shop_Forge.php?action=synth&item_id=' . $row['id']) . '" title="' . $lang['sshop_synthesize'] . ' ' . $row['name'] . '" class="navsmall"><b>' . $row['name'] . '</b></a>' : $row['name'];
	...
	$shoplocation = ' -> <a href="' . append_sid("shop_Forge.$phpEx") . '" class="nav">' . $page_title . '</a>';
	...
	$template->assign_vars(array(
		...
		'U_ACTION' => append_sid("shop_Forge.$phpEx")
	));
...
elseif ( $action == 'synth' )
{
	...
	{
		$redirect = 'shop_Forge.'.$phpEx.'&action=synth&item_id=' . $item_id;
	...
	if ( $sql_count )
	{
		...
		{
			$sql = '';
			message_die(GENERAL_MESSAGE, $error . sprintf($lang['sshop_return'], '<a href="' . append_sid('shop_Forge.php') . '" class="nav">', '</a>'));
		}
		else
		{
		...
		message_die(GENERAL_MESSAGE, sprintf($lang['sshop_synth_item'], $row['name']) . '!<br /><br />' . sprintf($lang['sshop_return'], '<a href="' . append_sid("shop_Forge.$phpEx") . '" class="nav">', '</a>'));
	}
	else
	{
...
elseif ( $action == 'desynth' )
{
	...
	if ( !$userdata['session_logged_in'] )
	{
		$redirect = 'shop_Forge.'.$phpEx.'&action=desynth&item_id=' . $item_id;
		header('Location: ' . append_sid("login.$phpEx?redirect=$redirect", true));
	}

	if ( !is_numeric($item_id) || $item_id < 0 ) { header("Location: shop_Forge.php"); }
	...
	if ( $sql_count )
	{
		...
		message_die(GENERAL_MESSAGE, sprintf($lang['sshop_desynth'], $row['item_name'], $new_items) . '!<br />' . sprintf($lang['sshop_return'], '<a href="' . append_sid("shop_Forge.$phpEx") . '" class="nav">', '</a>'));
	}
	else
	{
