...
for($i = 0; $i < $total_posts; $i++)
{
	...
	if ( $poster_id != ANONYMOUS )
	{
		...
		$mini_profile = '<a href="' . $mini_profile_url . '" onclick="window.open(\'' . $mini_profile_url . '\', \'_miniprofile\', \'height=310,width=300,resizable=yes,scrollbars=yes\');return false;" target="_miniprofile">' . $poster . '</a>';
		// Mini Profile End
	...
	$row_class = ( !($i % 2) ) ? $theme['td_class1'] : $theme['td_class2'];
	  ...
	  if ( $board_config['store_view_topic'] )
  	{
		$temp_url = append_sid("store.$phpEx?mode=user_items&id=" . $poster_id);
		$user_items = '<a href="' . $temp_url . '" class="postdetails">' . $lang['Store_users_items_goto'] . '</a>';
  	}
	else
	{
		$user_items = '';
			...
			if ($medal_list)
			{
				...
				while (list($medal_image, $medal) = each($rowset))
				{
					...
					$template->assign_block_vars('postrow.medal.medal_row.medal_col', array(
						...
						'MEDAL_NAME' => htmlentities($medal['medal_name']),
