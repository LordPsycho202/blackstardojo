...
{
	mo_echo_header($mo_lang['pmsent'], 'login.php', $mo_var['refresh_timer'], 
	    'redirect', 'pmsent.php', 'index', $opt['index']);
	mo_echo_paragraph_begin();
	mo_echo_msg($mo_lang['login_entry']);
	mo_echo_br();
	mo_echo_url($mo_lang['login'], 'login.php', 'redirect', 'pmsent.php', 
	    'index', $opt['index']);
...
$rows = array();
$rows = mo_select_privmsgs('privmsgs_sentbox', $userdata['user_id'], $opt['index']);

$num = count($rows);
if ( !$num )
{
	mo_echo_header($mo_lang['pmsent']);
	mo_echo_paragraph_begin();
...
for ( $i = 0; $i < $num1; $i++ )
{
	...
	mo_echo_url($rows[$i]['privmsgs_subject'], 'pmview.php', 
	    'privmsgs_id', $rows[$i]['privmsgs_id'], 'privmsgs_box', 'pmsent');
}
...
{
	mo_echo_url($mo_lang['next'], 'pmsent.php', 
...
if ( $opt['index'] >= $mo_var['row_size'] )
{
	...
	mo_echo_url($mo_lang['prev'], 'pmsent.php', 
	    'index', $opt['index'] - $mo_var['row_size']);
}
