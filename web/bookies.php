...
if (!$userdata['session_logged_in'] && $user_id == ANONYMOUS)
{
	...
	header($header_location . append_sid("login.$phpEx?redirect=bookies.$phpEx", true));
	exit;
}
...
$all_stats = '<a href="' . append_sid('bookie_allstats.'.$phpEx) . '"><img src="' . $images['icon_bookie_allstats'] . '" title="' . $lang['bookie_allstats_title'] . '" alt="' . $lang['bookie_allstats_title'] . '" border="0"></a>';
...
if ( $mode == 'delete' ) 
{
   ...
   $sql=" SELECT bet, user_id, admin_betid, time FROM " . BOOKIE_BETS_TABLE . "
	WHERE bet_id=$bet_id
	";
	...
	else if ( isset($HTTP_POST_VARS['confirm']) )
	{
		...
		$sql=" SELECT multi FROM " . BOOKIE_ADMIN_BETS_TABLE . "
...
if ( !$mode )
{
	...
	if ( $board_config['bookie_edit_stake'] == 1 )
	{
		if ( isset($HTTP_POST_VARS['change_stake'] ) )
		{
			...
			$sql=" SELECT bet_id,time,bet FROM " . BOOKIE_BETS_TABLE . "
			...
			while ( $row = $db->sql_fetchrow($result) )
			{
				...
				if ( !empty($HTTP_POST_VARS[$change_betid . $stake_name] ) )
				{
					...
					// Remove the commission for this bet
					$sql_z=" SELECT admin_betid FROM " . BOOKIE_BETS_TABLE . "
					WHERE bet_id=$change_betid
					";
					...
					$sql_z=" SELECT multi FROM " . BOOKIE_ADMIN_BETS_TABLE . "
	...
	$sql = "SELECT * FROM " . BOOKIE_STATS_TABLE . " ORDER BY netpos";
	if ( !($result = $db->sql_query($sql)) ) 
	{ 
	...
	while ( $row = $db->sql_fetchrow($result) )
	{
		...
		$sql = "UPDATE " . BOOKIE_STATS_TABLE . " SET netpos='$net_position' WHERE user_id='$lead_user_ID' ";
		if (!$db->sql_query($sql))
		{ 
	...
	$sql = "SELECT * FROM " . BOOKIE_STATS_TABLE . " ORDER BY netpos DESC";
	if ( !($result = $db->sql_query($sql)) ) 
	{ 
		message_die(GENERAL_ERROR, 'Error in getting leaderboard data', '', __LINE__, __FILE__, $sql); 
	}
	...
	while ( $row = $db->sql_fetchrow($result) )
	{
		...
		if ( $x < $leader_limit || $x == $leader_limit )
		{
			...
			$sql_a = "SELECT username FROM " . USERS_TABLE . " WHERE user_id='$lead_user_ID'";
			...
			'ROW_CLASS' => $row_class,
			'POSITION' => $x,
			'USERNAME' => '<a href=' . append_sid("bookie_yourstats.$phpEx?viewmode=viewuser&u=$lead_user_ID") . ' class="nav">' . $bet_username . '</a>',
	...
	// get list of bets this user has put on
		...
		while ( $row = $db->sql_fetchrow($result) )
		{
			...
			$row_color = ( !($pen_x % 2) ) ? $theme['td_color1'] : $theme['td_color2']; 
			...
			'DEL_IMG' => ( $board_config['bookie_edit_stake'] == 1 ) ? '<a href="' . $delete_url . '"><img src="' . $images['icon_delpost'] . '" alt="' . $lang['bookie_delete_bet'] . '" title="' . $lang['bookie_delete_bet'] . '" border="0" /></a>' : $pen_x,
			));
			$pen_x++;
