...
//
$params = array('submit' => 'post', 'preview' => 'preview', 'delete' => 'delete', 'poll_delete' => 'poll_delete', 'poll_add' => 'add_poll_option', 'poll_edit' => 'edit_poll_option', 'mode' => 'mode', 'itemid' => 'item_id', 'target' => 'target');
...
else if ( $submit || $confirm )
{
	...
	switch ( $mode )
	{
		...
		case 'reply':
			...
			if(($mode=="newtopic" || $mode="reply") && $itemid != 'none')
			{
				$mode="reply";
				$username = "Blackstar Foundation";
				$sql = "SELECT * FROM phpbb_shopitems WHERE 'item_id' = $itemid";
				...
				$broke = rand(1,100);
				$input = array("%U", "%T");
				$output = array($userdata['user_id'], $target);
				$subject = 'Item Usage Outcome';
				if($hit<$item['Prob'])
				{
					$message = str_replace($input, $output, $item['pass']);
				} else {
					$message = str_replace($input, $output, $item['fail']);
				}

				if($broke<$item['Break'])
				{
					$message = $message ." ".$item['btext'];
				}
				...
				submit_item_post($mode, $post_data, $return_message, $return_meta, $forum_id, $topic_id, $post_id, $poll_id, $topic_type, $bbcode_on, $html_on, $smilies_on, $attach_sig, $bbcode_uid, str_replace("\'", "''", $subject), str_replace("\'", "''", $message), str_replace("\'", "''", $poll_title), $poll_options, $poll_length);
				}
