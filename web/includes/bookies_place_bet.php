...
if ( !defined('IN_PHPBB') )
{
	...
	if ( $board_config['bookie_user_bets'] )
	{
		...
		if ( $condition == 'yes' && $mode_type == 'user' )
		{
			...
			if ( isset($HTTP_POST_VARS['user_placebet']) )
			{
				...
				$sql=" INSERT INTO " . BOOKIE_ADMIN_BETS_TABLE . "
				(bet_time,bet_selection,bet_meeting,odds_1,odds_2,multi,each_way)
				VALUES
				($bet_timestamp_placed,'" . str_replace("\'", "''", $bet_selection_placed) . "','" . str_replace("\'", "''", $bet_meeting_placed) . "',1,1,-5,'$eachway')
				";
				if ( !$db->sql_query($sql) ) 
				{ 
					message_die(GENERAL_ERROR, 'Error in entering new bet', '', __LINE__, __FILE__, $sql); 
				...
				//
				$sql=" INSERT INTO " . BOOKIE_BETS_TABLE . "
				(user_id,time,meeting,selection,bet,odds_1,odds_2,admin_betid,each_way)
				VALUES
				(" . $userdata['user_id'] . ",$bet_timestamp_placed,'" . str_replace("\'", "''", $bet_meeting_placed) . "','" . str_replace("\'", "''", $bet_selection_placed) . "',$user_bet_stake,1,1,$new_betid,$eachway)
				";
				if ( !$db->sql_query($sql) ) 
				{ 
					message_die(GENERAL_ERROR, 'Error in entering new bet', '', __LINE__, __FILE__, $sql); 
	...
	if ( $cat_view || $cat_count == 1 )
	{
		...
		while ( $row = $db->sql_fetchrow($result) )
		{
			...
			{
				$this_restriction=0;
				$sql_rest=" SELECT bet_id FROM " . BOOKIE_ADMIN_BETS_TABLE . "
				...
				{
					$sql_res=" SELECT admin_betid FROM " . BOOKIE_BETS_TABLE . "
	...
	//
	$cat_ids=array();
	$sql_cat=" SELECT * FROM " . BOOKIE_CAT_TABLE . "
	...
	while ( $row_cat = $db->sql_fetchrow($result_cat) )
	{
		if ( in_array($row_cat['cat_id'], $base_cat) )
		{
			...
			//
			$sql=" SELECT count(*) AS meetings FROM " . BOOKIE_ADMIN_BETS_TABLE . "
			WHERE multi=-1
			AND bet_time>$time_now
			AND checked=0
			AND bet_cat=$this_id
			...
			if ( $cat_view == $this_id || $cat_count == 1 )
			{
				...
				$x=1;
				$eachway_available=0;
				for ( $i=0; $i<count($base_meeting); $i++ )
				{ 
					if ( $base_eachway[$i] )
					{
						$eachway_available=1;
				...
				for ( $i=0; $i<count($base_meeting); $i++ )
				{
					...
					if ( !$base_restriction[$i] )
					{
						...
						$sql=" SELECT admin_betid FROM " . BOOKIE_BETS_TABLE . "
						...
						else
						{
							...
							{
								$sql=" SELECT count(*) AS placed FROM " . BOOKIE_BETS_TABLE . "
								WHERE admin_betid=$this_base_betid
								AND user_id=$userID
								";
								if ( !($result = $db->sql_query($sql)) ) 
						...
						while ( $row=$db->sql_fetchrow($result) )
						{
							...
							// now check to see if user has this selection placed
							$userID=$userdata['user_id'];
							$sql_a=" SELECT admin_betid FROM " . BOOKIE_BETS_TABLE . "
						 ...
						 if ( empty($row_a['admin_betid']) )
							...
							else
							{
								...
								if ( $base_eachway[$i] && $board_config['bookie_eachway'] )
								{
									$sql_z=" SELECT count(*) AS placed FROM " . BOOKIE_BETS_TABLE . "
									WHERE admin_betid=$this_multi_betid
									AND user_id=$userID
									";
						...
						'STAR' => ( $base_star[$i] ) ? '<img src="' . $images['icon_bookie_star'] . '" alt="' . $lang['bookie_star_alt'] . '" title="' . $lang['bookie_star_alt'] . '" border="0" />' : '',
	...
	if ( isset($HTTP_POST_VARS['placebet']) )
	{
		...
		for ( $i=0; $i<count($base_betid); $i++ )
		{
			...
			if ( !empty($HTTP_POST_VARS[$x]) && !empty($HTTP_POST_VARS[$base_timestamp[$i] . $base_betid[$i]]) )
			{
				...
				{
					$redirect='<meta http-equiv="refresh" content="5;url=' . append_sid("bookies.$phpEx?") . '">';
				...
				(user_id,time,meeting,selection,bet,odds_1,odds_2,admin_betid,each_way)
				VALUES
				($this_userid,$this_timestamp,'" . addslashes($this_meeting) . "','" . str_replace("\'", "''", $this_selection) . "',$this_stake,$this_odds1,$this_odds2,$this_admin_betid,'$each_way_selected')
				";
				if (!$db->sql_query($sql))
				{ 
				...
				if ( $board_config['bookie_allow_commission'] )
				{
					...
					$sql=" SELECT setter FROM " . BOOKIE_BET_SETTER_TABLE . "
