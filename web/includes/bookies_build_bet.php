...
else if ( !isset($HTTP_GET_VARS['field_count']) )
{
	...
	else if ( $use_templ )
	{
		...
		$selection_count=count($selection_value);
...
else
{
	...
	if ( $bet_id )
	{
		...
		for ( $i=1; $i<($fields+1); $i++ )
		{
			...
			if ( $selection && $odds1_placed && $odds2_placed )
			{
				...
				// process the bets
				//					
				$sql=" SELECT bet_time,bet_cat,bet_meeting,starbet,each_way FROM " . BOOKIE_ADMIN_BETS_TABLE . "
				...
				$sql = "INSERT INTO " . BOOKIE_ADMIN_BETS_TABLE . " (bet_time,bet_cat,bet_meeting,bet_selection,odds_1,odds_2,multi,starbet,each_way) VALUES ($multi_bet_time,$this_category,'" . addslashes($multi_bet_meeting) . "','" . str_replace("\'", "''", $selection) . "',$odds1_placed,$odds2_placed,$create_multi,'$this_starbet','$this_eachway')";
				if (!$db->sql_query($sql))
				{ 
	...
	else if ( $bet_time )
	{
		...
		//
		$selection_input=array();
		$odds1_input=array();
		$odds2_input=array();
		$odds_dec_input=array();
		$fields=intval($HTTP_GET_VARS['field_count']);
		for ( $i=1; $i<($fields+1); $i++ )
		{
			$selection_var='selection_' . $i;
		...
		$sql = "INSERT INTO " . BOOKIE_ADMIN_BETS_TABLE . " (bet_time,bet_cat,bet_meeting,bet_selection,odds_1,odds_2,starbet,each_way) VALUES ($bet_timestamp_placed,$bet_category,'" . addslashes($bet_meeting_placed) . "','" . str_replace("\'", "''", $bet_selection_placed) . "',$odds1_placed,$odds2_placed,$star_bet,$eachway)";
		if (!$db->sql_query($sql))
		{ 
			message_die(GENERAL_ERROR, 'Error in entering bet details into the DB', '', __LINE__, __FILE__, $sql); 
		}
		...
		if ( $board_config['bookie_allow_commission'] )
		{
			...
			$sql=" INSERT INTO " . BOOKIE_BET_SETTER_TABLE . "
			(bet_id, setter) VALUES ($setter_id, " . $userdata['user_id'] . ")
			";
			if ( !$db->sql_query($sql) ) 
			{ 
		...
		if ( count($selection_input) > 1 )
		{
			...
			for ( $i=1; $i<count($selection_input); $i++ )
			{
				$selection=$selection_input[$i];
				$odds1_placed=$odds1_input[$i];
				$odds2_placed=$odds2_input[$i];;
	
				$sql = "INSERT INTO " . BOOKIE_ADMIN_BETS_TABLE . " (bet_time,bet_cat,bet_meeting,bet_selection,odds_1,odds_2,multi,starbet,each_way) VALUES ($bet_timestamp_placed,$bet_category,'" . addslashes($bet_meeting_placed) . "','" . str_replace("\'", "''", $selection) . "',$odds1_placed,$odds2_placed,$create_multi,'$star_bet','$eachway')";
				if (!$db->sql_query($sql))
				{ 
					message_die(GENERAL_ERROR, 'Error in entering bet details into the DB', '', __LINE__, __FILE__, $sql); 
				}
