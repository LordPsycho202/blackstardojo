...
if( !empty($setmodules) )
{
	...
	$module['Bookmakers']['Process Bets'] = $filename;
...
// pay commission
$sql_a=" SELECT bet_id FROM " . BOOKIE_ADMIN_BETS_TABLE . "
WHERE bet_time<" . time() . "
AND checked=0
AND multi = -1
";
...
while ( $row_a = $db->sql_fetchrow($result_a) )
{
	...
	$sql=" SELECT commission, setter, paid FROM " . BOOKIE_BET_SETTER_TABLE . "
...
while ( $row = $db->sql_fetchrow($result) )
{
	...
	else
	{
		...
		if ( $row_a )
		{
			 $sql_b=" INSERT INTO " . BOOKIE_ADMIN_BETS_TABLE . "
			 (bet_time,bet_selection,bet_meeting,odds_1,odds_2,multi,each_way)
			 VALUES
			 (" . $row_a['time'] . ",'" . addslashes($row_a['selection']) . "','" . addslashes($row_a['meeting']) . "'," . $row_a['odds_1'] . "," . $row_a['odds_2'] . ",-2,-1)
			 ";
...
for ( $loop=0; $loop<count($this_betid_arr); $loop++ )
{
	...
	if ( !empty($enter_betid) )
	{
		...
		{
			$sql = "SELECT * FROM " . BOOKIE_ADMIN_BETS_TABLE . " WHERE bet_id=$enter_betid ";
			if ( !($result = $db->sql_query($sql)) ) 
			{ 
