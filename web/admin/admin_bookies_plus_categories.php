...
if( !empty($setmodules) )
{
	...
	$module['Bookmakers +']['Categories'] = $filename;
...
//
$sql=" SELECT cat_name FROM " . BOOKIE_CAT_TABLE . "
WHERE cat_id=1
";
...
if ( isset($HTTP_POST_VARS['delete_marked']) )
{
	...
	//
	$sql=" SELECT cat_id FROM " . BOOKIE_CAT_TABLE;
	if ( !($result = $db->sql_query($sql)) ) 
	{ 
...
if ( $mode == 'add' )
{
	...
	else if ( !empty($HTTP_POST_VARS['add_cat']) )
	{
		...
		$sql=" INSERT INTO " . BOOKIE_CAT_TABLE . "
		(cat_name) VALUES ('" . str_replace("\'", "''", $new_cat) . "')
		";
		if ( !$db->sql_query($sql) ) 
		{ 
...
while ( $row = $db->sql_fetchrow($result) )
	{
		...
		'EDIT_IMG' => '<a href="' . $edit_url . '"><img src="' . $phpbb_root_path . $images['icon_edit'] . '" alt="' . $lang['bookie_edit_category'] . '" title="' . $lang['bookie_edit_category'] . '" border="0" /></a>',
		));
		$x++;
...
'IMG_NEW_CATEGORY' => '<a href="' . $add_category_url . '"><img src="' . $phpbb_root_path . $images['icon_bookie_add_category'] . '" alt="' . $lang['icon_bookie_add_category'] . '" title="' . $lang['icon_bookie_add_category'] . '" border="0" /></a>',
