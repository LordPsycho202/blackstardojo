...
if ( isset($HTTP_GET_VARS['ammo']) || isset($HTTP_POST_VARS['ammo']) ) { $ammo = ( isset($HTTP_POST_VARS['ammo']) ) ? intval($HTTP_POST_VARS['ammo']) : intval($HTTP_GET_VARS['ammo']); }
	else { $ammo = ''; }
	if ( isset($HTTP_GET_VARS['pass']) || isset($HTTP_POST_VARS['pass']) ) { $pass = ( isset($HTTP_POST_VARS['pass']) ) ? intval($HTTP_POST_VARS['pass']) : intval($HTTP_GET_VARS['pass']); }
	else { $pass = ''; }
	if ( isset($HTTP_GET_VARS['fail']) || isset($HTTP_POST_VARS['fail']) ) { $fail = ( isset($HTTP_POST_VARS['fail']) ) ? intval($HTTP_POST_VARS['fail']) : intval($HTTP_GET_VARS['fail']); }
	else { $fail = ''; }
	if ( isset($HTTP_GET_VARS['btext']) || isset($HTTP_POST_VARS['btext']) ) { $btext = ( isset($HTTP_POST_VARS['btext']) ) ? intval($HTTP_POST_VARS['btext']) : intval($HTTP_GET_VARS['btext']); }
	else { $btext = ''; }
	if ( isset($HTTP_GET_VARS['prob']) || isset($HTTP_POST_VARS['prob']) ) { $prob = ( isset($HTTP_POST_VARS['prob']) ) ? intval($HTTP_POST_VARS['prob']) : intval($HTTP_GET_VARS['prob']); }
	else { $prob = '0'; }
	if ( isset($HTTP_GET_VARS['break']) || isset($HTTP_POST_VARS['break']) ) { $break = ( isset($HTTP_POST_VARS['break']) ) ? intval($HTTP_POST_VARS['break']) : intval($HTTP_GET_VARS['break']); }
	else { $break = '0'; }
	...
	if ( $action == 'additem' )
	{
		...
		$sql = "INSERT INTO " . SHOP_ITEMS_TABLE . "
			(name, shop, sdesc, ldesc, cost, stock, maxstock, sold, ammo, pass, fail, Prob, Break, btext)
			VALUES('" . str_replace("\'", "''", $item) . "', '" . str_replace("'", "''", $row['shopname']). "', '" . str_replace("\'", "''", $shortdesc) . "', '" . str_replace("\'", "''", $longdesc) . "', $price, $stock, $maxstock, 0, '" . str_replace("\'","''",$ammo)."' , '".str_replace("\'","''",$pass)."','".str_replace("\'","''",$fail)."',$prob,$break,'".str_replace("\'","''",$btext)."')";
		if ( !($db->sql_query($sql)) )
		{
	...
	elseif ( $action == 'updateitem' )
	{
		...
		if ( isset($HTTP_GET_VARS['ammo']) || isset($HTTP_POST_VARS['ammo']) ) { $ammo = ( isset($HTTP_POST_VARS['ammo']) ) ? $HTTP_POST_VARS['ammo'] : $HTTP_GET_VARS['ammo']; }
		else { $ammo = ''; }
		...
		if ( isset($HTTP_GET_VARS['btext']) || isset($HTTP_POST_VARS['btext']) ) { $btext = ( isset($HTTP_POST_VARS['btext']) ) ? $HTTP_POST_VARS['btext'] : $HTTP_GET_VARS['btext']; }
		else { $btext = ''; }
		if ( isset($HTTP_GET_VARS['prob']) || isset($HTTP_POST_VARS['prob']) ) { $prob = ( isset($HTTP_POST_VARS['prob']) ) ? intval($HTTP_POST_VARS['prob']) : intval($HTTP_GET_VARS['prob']); }
		else { $prob = '0'; }
		if ( isset($HTTP_GET_VARS['break']) || isset($HTTP_POST_VARS['break']) ) { $break = ( isset($HTTP_POST_VARS['break']) ) ? intval($HTTP_POST_VARS['break']) : intval($HTTP_GET_VARS['break']); }
...
elseif ( $action == 'editshop' )
{
	...
	$template->assign_vars(array(
		...
		'L_PASS' => "Hit Text",
		'L_FAIL' => "Miss Text",
		'L_BREAK' => "Breaking Text",
		'L_PROB' => "Probability of a hit, 1-100",
		'L_BPROB' => "Probability of item breaking, 1-100"
	));
}
...
elseif ( $action == 'edititem' )
{
	...
	if ( $row['shop'] == 'Synthesize Shop' || $row['shop'] == 'Back_Forge' || $row['shop'] == 'Back_Mill' || $row['shop'] == 'Back_Firepit' || $row['shop'] == 'Back_Jewel' || $row['shop'] == 'Back_Bake' || $row['shop'] == 'Back_Blacksmith' || $row['shop'] == 'Back_Research' || $row['shop'] == 'Back_Shaman' || $row['shop'] == 'Back_Butcher' || $row['shop'] == 'Back_Craft')
	{
	...
	$template->assign_vars(array(
		...
		'ITEM_PASS' => $row['pass'],
		'ITEM_FAIL' => $row['fail'],
		'ITEM_BTEXT' => $row['btext'],
		'ITEM_PROB' => $row['Prob'],
		'ITEM_BREAK' => $row['Break'],
...
elseif ( $action == 'updateinv' )
{
	...
	if ( isset($HTTP_GET_VARS['quantity']) || isset($HTTP_POST_VARS['quantity']) ) { $quantity = ( isset($HTTP_POST_VARS['quantity']) ) ? intval($HTTP_POST_VARS['quantity']) : intval($HTTP_GET_VARS['quantity']); }
	else { $quantity = ''; }
	...
	elseif ( $subaction == 'add' )
	{
		...
		for($i=0;$i<$quantity;$i++)
		{
			...
			VALUES({$user_row['user_id']}, {$row['id']}, '" . str_replace("'", "''", $row['name']) . "', '" . str_replace("'", "''", $row['sdesc']) . "', '" . str_replace("'", "''", $row['ldesc']) . "' , '".str_replace("'","''",$row['ammo'])."', '".str_replace("'","''",$row['pass'])."', '".str_replace("'","''",$row['fail'])."', ".$row['Prob'].", ".$row['Break'].", '".str_replace("'","''",$row['btext'])."')";
			if ( !($db->sql_query($sql)) )
			{
		...
		$message = sprintf($quanity.$lang['ashop_user_item_added'], $row['name'], str_replace("s's", "s'", $user_row['username'] . "'s")) . '<br /><br />' . sprintf($lang['ashop_click_inventory'], '<a href="'.append_sid("admin_shop.".$phpEx."?username=" . $user_row['user_id'] . "&action=editinventory").'">','</a>', str_replace("s's", "s'", $user_row['username'] . "'s")) . '<br /><br />' . sprintf($lang['ashop_click_index'], '<a href="' . append_sid("admin_shop.".$phpEx) . '">','</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . append_sid("index.$phpEx?pane=right") .'">', '</a>');
		message_die(GENERAL_MESSAGE, $message);
	}
