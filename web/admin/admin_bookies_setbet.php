...
if( !empty($setmodules) )
{
	...
	$module['Bookmakers']['Admin Set Bet'] = $filename;
...
if ( $mode == 'selection_review' )
{
	...
	while ( $expand_data = $db->sql_fetchrow($result_expand) )
	{
		...
		'STAR_IMAGE' => ( $star_status ) ? '<img src="' . $phpbb_root_path . $images['icon_bookie_star'] . '" height="20" width="20" alt="' . $lang['bookie_star_alt'] . '" title="' . $lang['bookie_star_alt'] . '" border="0" />' : '',
		));
		$x++;
	}
	//
...
while ( $bet_data = $db->sql_fetchrow($result) )
{
	...
	'TIME' => $bet_time,
	'MEETING' => $bet_meeting,
	'STAR_IMAGE' => ( $star_status ) ? '<img src="' . $phpbb_root_path . $images['icon_bookie_star'] . '" height="15" width="15" alt="' . $lang['bookie_star_alt'] . '" title="' . $lang['bookie_star_alt'] . '" border="0" />' : '',
	'ADD_IMAGE' => '<a href="' . $add_url . '"><img src="' . $phpbb_root_path . $images['icon_bookie_add_selection'] . '" alt="' . $lang['icon_bookie_add_selection'] . '" title="' . $lang['icon_bookie_add_selection'] . '" border="0" /></a>',
	'DROP_IMAGE' => '<a href="' . $drop_url . '"><img src="' . $phpbb_root_path . $images['icon_bookie_drop_meeting'] . '" alt="' . $lang['icon_bookie_drop_meeting'] . '" title="' . $lang['icon_bookie_drop_meeting'] . '" border="0" /></a>',
	...
	if ( isset($HTTP_GET_VARS['expand']) )
	{
		...
		if ( $expand_id == $bet_id )
		{
			...
			'SELECTION' => $bet_selection,
			'ODDS' => ( $odds_decimal ) ? $odds_decimal : $bet_odds1 . '/' . $bet_odds2,
			'EDIT' => '<a href="' . $edit_url . '"><img src="' . $phpbb_root_path . $images['icon_edit'] . '" alt="' . $lang['bookie_edit_bet'] . '" title="' . $lang['bookie_edit_bet'] . '" border="0" /></a>',
			'DELETE' => '<a href="' . $delete_url . '"><img src="' . $phpbb_root_path . $images['icon_delpost'] . '" alt="' . $lang['bookie_delete_bet'] . '" title="' . $lang['bookie_delete_bet'] . '" border="0" /></a>',
			'STAR_IMAGE' => ( $star_status ) ? '<img src="' . $phpbb_root_path . $images['icon_bookie_star'] . '" height="20" width="20" alt="' . $lang['bookie_star_alt'] . '" title="' . $lang['bookie_star_alt'] . '" border="0" />' : '',
			'CATEGORY' => $cat_name[$bet_cat],
			));
			$x++;
...
'IMG_NEW_MEETING' => '<a href="' . $add_meeting_url . '"><img src="' . $phpbb_root_path . $images['icon_bookie_add_meeting'] . '" alt="' . $lang['icon_bookie_add_meeting'] . '" title="' . $lang['icon_bookie_add_meeting'] . '" border="0" /></a>',
//
// 2.0.8
//
