...
//
//error_log("[DEBUG] pre-left");
//$isset=isset($HTTP_GET_VARS['pane']);
//error_log("[DEBUG] isset($ HTTP_GET_VARS[pane])=".$isset);
//if($isset)
//{
//	$pane=$HTTP_GET_VARS['pane'];
//	error_log("[DEBUG] $ HTTP_GET_VARS[pane]=".$pane);
//}
if( isset($HTTP_GET_VARS['pane']) && $HTTP_GET_VARS['pane'] == 'left' )
{

	//error_log("[DEBUG] POST LEFT");
	$dir = opendir(".");

	$setmodules = 1;
	while( $file = readdir($dir) )
	{
		//error_log("[DEBUG] $ file =".$file);
		if( preg_match("/^admin_.*?\." . $phpEx . "$/", $file) )
		{
			include('./' . $file);
			//error_log("[DEBUG] Admin file ".$file);
		}
	}
	//error_log("[DEBUG] end while");
	closedir($dir);

	unset($setmodules);

	//error_log("[DEBUG] include header");
