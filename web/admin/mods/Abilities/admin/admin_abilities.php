...
if ( empty($action) )
{
	...
	for ( $i = 0; $i < $sql_num_rows; $i++ )
	{
		...
		$template->assign_block_vars('list_abilities', array(
			...
			'pre2' => $pre2,
			'trade' => $trade,
			"U_ABILITY_EDIT" => append_sid("admin_ability.$phpEx?action=edit&amp;id=" . $row['id']),
			"U_ABILITY_DELETE" => append_sid("admin_ability.$phpEx?action=delete&amp;id=" . $row['id'])
		));
...
elseif ( $action == 'updatestatus' )
{
	...
	$status = ( ($status < 0) ) ? $board_config['abilities_enabled'] : $status;
...
elseif ( $action == 'add' || $action == 'update' )
{
	...
	if ( $action == 'add' )
	{
		...
		$sql = "INSERT INTO " . ABILITIES_TABLE . "
			(name, desc, start, max, master, inc, pre1, pre2, trade_items)
			VALUES('" . str_replace("\'", "''", $name) . "',  '" . str_replace("\'", "''", $desc) . "', $start, $max, $master, $inc, $pre1, $pre2, $trade)";
		if ( !($db->sql_query($sql)) )
		{
			message_die(GENERAL_MESSAGE, 'SQL Error adding item to shops!');
		}

		$message = 'Created Ability'. $name . '<br /><br />' . '<a href="' . append_sid("admin_shop.".$phpEx) . '">Return to Abilities page</a>' . '<br /><br />' . '<a href="' . append_sid("index.$phpEx?pane=right") .'">Return to Admin Index</a>';
		message_die(GENERAL_MESSAGE, $message);
	}
	elseif ( $action == 'update' ). '<br /><br />' .  '<a href="' . append_sid("admin_shop.".$phpEx) . '">Return to Abilities page</a>' . '<br /><br />' . '<a href="' . append_sid("index.$phpEx?pane=right") .'">Return to Admin Index</a>';
	{
		...
		# Update User abilities
  		$sql = "UPDATE " . USER_ABILITIES_TABLE . "
		...
		$message = 'Ability $name updated' . '<br /><br />' .  '<a href="' . append_sid("admin_shop.".$phpEx) . '">Return to Abilities page</a>' . '<br /><br />' . '<a href="' . append_sid("index.$phpEx?pane=right") .'">Return to Admin Index</a>';
		message_die(GENERAL_MESSAGE, $message);
	}
}
...
elseif ( $action == 'delete' )
{
	...
	$message = Ability Baleeted! . . '<br /><br />' .  '<a href="' . append_sid("admin_shop.".$phpEx) . '">Return to Abilities page</a>' . '<br /><br />' . '<a href="' . append_sid("index.$phpEx?pane=right") .'">Return to Admin Index</a>';
	message_die(GENERAL_MESSAGE, $message);
}
