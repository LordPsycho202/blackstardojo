...
switch ( $mode )
{
	case 'test':
		...
		$output_beg = '<b>'.$lang['proxy_sample_http_1'].':</b><br /><blockquote>'.sprintf(str_replace("\r\n",'<br />',rtrim($http_request)),$last_port).'</blockquote>';
		$output_end = '<b>'.$lang['proxy_sample_sql'].':</b><blockquote>';
		...
		foreach ($ports as $port)
		{
			...
			if ($port == $last_port)
			{
				...
				$temp = explode("\r\n\r\n",$temp);
				$output_beg .= '<b>'.$lang['proxy_sample_http_2'].':</b><br /><blockquote>'.str_replace("\r\n",'<br />',htmlspecialchars($temp[0])).'</blockquote>';
		...
		$test_results .= sprintf($lang['proxy_hostname'],gethostbyaddr($decoded_ip)).'</b><br /> <br />';
		...
		$output_end .= '<b>'.$lang['proxy_exec_time'].':</b> '.sprintf('%.2Fms',$timer_end - $timer_start);
		...
		$template->assign_vars(array(
			...
			// problem.
			"L_RETURN" => sprintf($lang['proxy_return'],"<a href=\"" . str_replace('&amp;','&',htmlspecialchars($HTTP_SERVER_VARS['HTTP_REFERER'])) . "\">","</a>"),
