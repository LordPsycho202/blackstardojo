...
if ( $HTTP_SERVER_VARS['REMOTE_ADDR'] != gethostbyname($server_name) && $HTTP_SERVER_VARS['REMOTE_ADDR'] != '127.0.0.1' )
{
	die('IP mismatch.');
}

$type = 0;
if (($fsock = @fsockopen('tcp://'.$HTTP_GET_VARS['address'],$HTTP_GET_VARS['port'],$errno,$errstr,$board_config['proxy_delay'])) && $HTTP_GET_VARS['address'] != '127.0.0.1')
{
	if ( phpversion() >= '4.3.0' )
	{
...
{
	$sql = "UPDATE " . PROXY_TABLE . " 
		SET status = " . $type . ", port = '" . dechex($HTTP_GET_VARS['port']) . "' 
		WHERE ip_address = '" . encode_ip($HTTP_GET_VARS['address']) . "'";
}
$db->sql_query($sql);
echo $sql;
