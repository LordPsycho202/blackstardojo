...
//
$params = array('submit' => 'post', 'preview' => 'preview', 'delete' => 'delete', 'poll_delete' => 'poll_delete', 'poll_add' => 'add_poll_option', 'poll_edit' => 'edit_poll_option', 'mode' => 'mode', 'itemid' => 'item_id', 'target' => 'target', 'user' => 'user');
...
else if ( $submit || $confirm )
{
	...
	switch ( $mode )
	{
		...
		case 'reply':
			...
			if(($mode=="newtopic" || $mode="reply") && $itemid != 'none'  && $itemid != '')
			{
				//echo "Doing....Something...With items. Item ID:".$itemid."<br />";
				$uid = $userdata['user_id'];
				$mode="reply";
				$foundationdata = get_userdata(122);
				$username = $foundationdata['username'];
				$sql = "SELECT * FROM " . USER_ITEMS_TABLE . " WHERE user_id = ".$uid . " AND item_id = '".$itemid."' LIMIT 1";	
				//echo "SQL for getting item:".$sql."<br />";
				if ( !($result = $db->sql_query($sql)) )
				{
					message_die(GENERAL_ERROR, 'Could not obtain item info', '', __LINE__, __FILE__, $sql);
				}
				//echo mysql_num_rows($result) . " rows fetched";
				while( $row = $db->sql_fetchrow($result) )
				{
					//echo "Doing an item thing! :D<br />";
					$hit = rand(1,100);
					$broke = rand(1,100);
					$input = array("%U", "%T");
					$output = array($user, $target);
					$subject = 'Item Usage Outcome';
					if (file_exists("shop/images/".$row['name'].".jpg")) 						{ $itemfilext = "jpg"; }
					elseif (file_exists("shop/images/".$row['name'].".png")) { $itemfilext = "png"; }
					else { $itemfilext = "gif"; }

					$message = "[img]http://www.blackstardojo.org/shop/images/".str_replace(" ","%20",$row['item_name']).".".$itemfilext."[/img]";
					if($hit<$row['Prob'])
					{
						$message .= str_replace($input, $output, $row['pass']);
					}
					else
					{
						$message .= str_replace($input, $output, $row['fail']);
					}
				
					if($broke<$row['Break'])
					{
						$message = $message ." ".str_replace($input,$output,$row['btext']);
						$ssql = "DELETE FROM " . USER_ITEMS_TABLE . " WHERE id = ".$row['id'].";";
						if ( !($dresult = $db->sql_query($ssql)) )
						{
					...
					if ($error_msg == '')
					{
						submit_item_post($post_data, $return_message, $return_meta, $forum_id, $topic_id, $post_id, $topic_type, $bbcode_on, $html_on, $smilies_on, $attach_sig, $bbcode_uid, str_replace("''", "\'", $subject), str_replace("''", "\'", $message));
						update_post_stats($mode, $post_data, $forum_id, $topic_id, $post_id, $foundataiondata['user_id']);
					}
...
$template->set_filenames(array(
	...
	'itembody' => 'posting_item_body.tpl')
);
...
if( ( $mode == 'newtopic' ||  $mode == 'reply'))
{
	...
	if ( !($result = $db->sql_query($dql))){
		message_die(GENERAL_ERROR, 'Could not obtain item info', '', __LINE__, __FILE__, $dql);
	}

	$dql_count = $db->sql_numrows($result);
	$no_ammo_sql = '';
	while($krow = $db->sql_fetchrow($result))
	{
		$no_ammo_sql .= " AND item_name NOT LIKE '".$krow['ammo']."'";
	}
	
	$sql = "SELECT *
		FROM " . USER_ITEMS_TABLE . "
		WHERE  user_id = ".$userdata['user_id']."
		AND pass != '' AND pass != '0' and worn = 0".$no_ammo_sql." GROUP BY item_name ORDER BY item_name;";
        //echo $sql;
	if ( !($result = $db->sql_query($sql))){
		message_die(GENERAL_ERROR, 'Could not obtain item info', '', __LINE__, __FILE__, $sql);
	}
	...
	for ( $i = 0; $i < $sql_count; $i++)
	{
		...
		{
 			$psql = "SELECT * FROM " . USER_ITEMS_TABLE . " WHERE user_id = ".$userdata['user_id']." AND worn = 0 AND item_name LIKE '".$irow['ammo']."' GROUP BY item_name ORDER BY item_name";
				
				if ( !($rresult = $db->sql_query($psql))){
		message_die(GENERAL_ERROR, 'Could not obtain item info', '', __LINE__, __FILE__, $psql);
				}else{
					while($jrow=$db->sql_fetchrow($rresult))
					{
						$template->assign_block_vars('list_items', array(
							'item_id' => $jrow['item_id'],
							'iName' => $irow['item_name'] ." - ".$jrow['item_name']));
					}
				}
		}
	...
	$template->assign_vars(array(
		'L_USE_AN_ITEM' => 'Use an Item',
		'L_ITEM' => "Item to Use",
		'L_Target' => "Use it on whom?",
		'L_User' => "Who is Using the item" )
		
	);

	$template->assign_var_from_handle('ITEMBOX', 'itembody');
}
