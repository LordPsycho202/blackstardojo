 ...
 *
 *     $Id: bookie_yourstats.php,v 2.0.6 
 *
 ****************************************************************************/
...
$all_stats = '<a href="' . append_sid('bookie_allstats.'.$phpEx) . '"><img src="' . $images['icon_bookie_allstats'] . '" title="' . $lang['bookie_allstats_title'] . '" alt="' . $lang['bookie_allstats_title'] . '" hspace="3" border="0"></a>';
...
if (!$userdata['session_logged_in'] && $user_id == ANONYMOUS)
{
	...
	header($header_location . append_sid("login.$phpEx?redirect=bookie_yourstats.$phpEx", true));
	exit;
}
...
// Get Stats
$sql = "SELECT * FROM " . BOOKIE_BETS_TABLE . " WHERE user_id='$userID' ORDER BY $order_by"; 
if ( !($result = $db->sql_query($sql)) ) 
{ 
...
$sql = "SELECT MAX(bet) AS big_bet, MAX(win_lose) AS max, MIN(win_lose) AS min FROM " . BOOKIE_BETS_TABLE . "
WHERE user_id=" . $userdata['user_id'];
if ( !($result = $db->sql_query($sql)) ) 
{ 
...
FROM ' . BOOKIE_BETS_TABLE . " WHERE user_id='$userID' ORDER BY time DESC"; 
